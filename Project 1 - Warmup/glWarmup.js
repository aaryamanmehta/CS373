...
//Creates TorusKnot mesh
var torusKnot_geometry = new THREE.TorusKnotGeometry(2.641, 2.1681, 113, 9, 8, 4);
var torusKnot = new THREE.Mesh(torusKnot_geometry, material);
shapes.push(torusKnot);

//Creates TorusKnot2 mesh
var torusKnot_geometry2 = new THREE.TorusKnotGeometry(7.543, 2.1681, 72, 20, 5, 20);
var torusKnot2 = new THREE.Mesh(torusKnot_geometry2, material);
...
function render() {
   ...
   * Write your answer here in 1-2 sentences: The program uses the .setFromCamera and the .intersectObjects to detect if the mouse pointer is on the shape, as the .setFromCamera method
   checks the 2D coordinates of the mouse, and the .intersectObjects method checks if the object (which I am assuming to be the shape) is being intersected with the ray. The ray in this
   case is the ray created from the .setFromCamera method. If the intersection exists (i.e. length > 0), the variable clickedOnShape is set to true, and if not, it's set to false.
   *
   *********************************/
	if (mouseClicked) {
	...
	if (clickedOnShape && mouseButton == 0) { // update vertical and horizontal rotation angle
