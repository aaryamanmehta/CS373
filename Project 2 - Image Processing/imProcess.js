...
// ===YOUR CODE STARTS HERE===
	...
	for(let i=0; i<input.width*input.height; i++) {
		let idx = i*4; // each pixel takes 4 bytes: red, green, blue, alpha(not used in this homework)
		let L = 0.3*pixelClamp(ip[idx+0]) + 0.59*pixelClamp(ip[idx+1]) + 0.11*pixelClamp(ip[idx+2]);
...
// ===YOUR CODE STARTS HERE===
    ...
    let denominatorG = [];
    for(let k = 0; k < num; k++){
        let mk = mimages[k].pixels;
        for(let a = 0; a < h; a++){
            for(let b = 0; b < w; b++){
                let idx = (b + a*w)*4;
				result[0] = result[0] + mk[idx+0]*mk[idx+0];
				result[1] = result[1] + mk[idx+1]*mk[idx+1];
				result[2] = result[2] + mk[idx+2]*mk[idx+2];
            }
        }
        denominatorR.push(result[0]);
        denominatorG.push(result[1]);
        denominatorB.push(result[2]);
    }
// ---YOUR CODE ENDS HERE---
...
// ===YOUR CODE STARTS HERE===
        ...
        for(let k = 0; k < num; k++){
            ...
            for(let j = 0; j < h; j++){
                for(let i = 0; i < w; i++){
                    let idx = (i + j*w)*4;
                    let IPidx = ((x+i) + (y+j)*width) * 4;
                    a[0] = a[0] + (ip[IPidx+0]*mk[idx+0]);
                    a[1] = a[1] + (ip[IPidx+1]*mk[idx+1]);
                    a[2] = a[2] + (ip[IPidx+2]*mk[idx+2]);
                    dPixels[0] = dPixels[0] + (ip[IPidx+0]*mk[idx+0]);
                    dPixels[1] = dPixels[1] + (ip[IPidx+1]*mk[idx+1]);
                    dPixels[2] = dPixels[2] + (ip[IPidx+2]*mk[idx+2]);
                }
            }
            d = (-1*(dPixels[0] * dPixels[0])/denominatorR[k]) + (-1*(dPixels[1] * dPixels[1])/denominatorG[k]) + (-1*(dPixels[2] * dPixels[2])/denominatorB[k]);
			d = 1 + Math.random() * d;
            if(d <= smallD){
				smallD = d;
				lowestK = k;
		...
		for(let j = 0; j < h; j++){
			for(let i = 0; i < w; i++){
				let idx = (i + j*w)*4;
				let IPidx = ((x+i) + (y+j)*width) * 4;
				op[IPidx + 0] = a[0]/(denominatorR[lowestK])*mimages[lowestK].pixels[idx + 0];
				op[IPidx + 1] = a[1]/(denominatorG[lowestK])*mimages[lowestK].pixels[idx + 1];
				op[IPidx + 2] = a[2]/(denominatorB[lowestK])*mimages[lowestK].pixels[idx + 2];
			}
		}
// ---YOUR CODE ENDS HERE---
		}
